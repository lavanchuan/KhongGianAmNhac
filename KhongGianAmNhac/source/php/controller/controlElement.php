<?php 
	...
	switch($_SESSION['type']){
		case '1':
			if($submit == "Add"){
				$exec = addSong($_POST['txtIdSong'], $_POST['txtNameSong'], $_POST['txtEmailSinger'],
					$_POST['txtEmailComposer'], $_POST['txtReleaseTime'], basename($_FILES['src']['name']));
		...
		case '2':
			if($submit == "Add"){
				$exec = addSinger($_POST['txtEmail'], $_POST['txtNameSinger'], $_POST['txtBirthday'],
					$_POST['txtAddress'], basename($_FILES['image']['name']));
			} else if($submit == "Update"){
		...
		case '3':
			if($submit == "Add"){
				$exec = addComposer($_POST['txtEmail'], $_POST['txtNameComposer'], $_POST['txtBirthday'],$_POST['txtAddress'], basename($_FILES['image']['name']));
			} else if($submit == "Update"){
				$exec = updateComposer($_POST['txtEmail'], $_POST['txtNameComposer'], $_POST['txtBirthday'],$_POST['txtAddress'], basename($_FILES['image']['name']));
			} else {
